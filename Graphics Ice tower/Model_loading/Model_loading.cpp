...
// object #3 VBO and Decleration
    ...
    //#7 objects
        ...
        bool res7 = loadOBJ("gameover.obj", vertices7, uvs7, normals7);
        ...
        GLuint uvbuffer7;
        glGenBuffers(1, &uvbuffer7);
        glBindBuffer(GL_ARRAY_BUFFER, uvbuffer7);
        glBufferData(GL_ARRAY_BUFFER, uvs7.size() * sizeof(glm::vec2), &uvs2[0], GL_STATIC_DRAW);


        GLuint normalbuffer7;
        glGenBuffers(1, &normalbuffer7);
        glBindBuffer(GL_ARRAY_BUFFER, normalbuffer7);
        glBufferData(GL_ARRAY_BUFFER, normals7.size() * sizeof(glm::vec3), &normals7[0], GL_STATIC_DRAW);
    ...
    if (!dead)
    {
        ...
        glm::mat4 View       = glm::lookAt(
                                    glm::vec3(0,Ycamera+4,12), // Camera is at (0,Ycamera+4,12), in World Space
...
if (!dead)
{
      ...
      //jumbing down calculate if collision happens
        if (!UP||!jumping)
           {
                if (Ycurrent<=0)
                    {
    ...
    //handling collision with steps
                   ...
                   if(Ycurrent > steps[coll_step][1])
                   {
                       ...
                       if(Ycurrent-steps[coll_step][1]<=7.0*deltaTime && (mario_rotation > edge1 && mario_rotation < edge2))
                       {
        ...
        if (Ax+0.25f*deltaTime<1.0)
            Ax+=0.25f*deltaTime;


        glUseProgram(programID);
        GLuint Ambient_id = glGetUniformLocation(programID, "Ambient");
        GLuint LightID = glGetUniformLocation(programID, "LightPosition_worldspace");


        glm::vec4 lightPos = eulerAngleYXZ(mario_rotation, 0.0f,0.0f)*translate(mat4(), vec3(0,Ycamera+4,15))*glm::vec4(0.0,0.0,0.0,1);
        glUniform3f(LightID, lightPos.x, lightPos.y, lightPos.z);


        glm::vec3 Ambient = glm::vec3(Ax,Ax,Ax);
        glUniform3f(Ambient_id, Ambient.x, Ambient.y, Ambient.z);


        GLuint Light_POWER_ID = glGetUniformLocation(programID, "LightPower");

        if (lightPow<30)
            lightPow +=(30.0f/4)*deltaTime;

        glUniform1f(Light_POWER_ID, lightPow);
        ...
        if(sec-l_sec>1000)   // generate every minuite
   {
    ...
    if(Exist&&!dead)    //draw if if it exists
    {
        ...
        for(int i=0;i<=3;i++)
       {
        ...
        //#7 Game over objects
        if(scaler+0.9*deltaTime<1)
            scaler+=0.9*deltaTime;

            rot+=3.14*deltaTime;

        // Model matrix
            glm::mat4 RotationMatrix7 = eulerAngleYXZ(3.14f, -0.1f,0.0f);    //(*pai)/180
            glm::mat4 TranslationMatrix7 = translate(mat4(), vec3(0.0f,Ycamera+3.0f,-6.0f));
            glm::mat4 RotationMatrix8 = eulerAngleYXZ(3.14f+mario_rotation, 0.0f,0.0f);    //(*pai)/180
            glm::mat4 RotationMatrix9 = eulerAngleYXZ(0.0f,rot,0.0f);    //(*pai)/180
            glm::mat4 ScalingMatrix7 = scale(mat4(), vec3(scaler,scaler,scaler ));
            glm::mat4 Model7 =ScalingMatrix7*RotationMatrix8*TranslationMatrix7*RotationMatrix7*RotationMatrix9;

                // Our ModelViewProjection : multiplication of our 3 matrices
            glm::mat4 MVP7        = Projection* View * Model7; // Remember, matrix multiplication is the other way around
                                              //View of Camera

            // Send our transformation to the currently bound shader,
            // in the "MVP" uniform
            glUniformMatrix4fv(MatrixID, 1, GL_FALSE, &MVP7[0][0]);
            glUniformMatrix4fv(ModelMatrixID, 1, GL_FALSE, &Model7[0][0]);
            glUniformMatrix4fv(ViewMatrixID, 1, GL_FALSE, &View[0][0]);
            ...
            // Draw the triangles !
            glDrawArrays(GL_TRIANGLES, 0, vertices7.size() );
